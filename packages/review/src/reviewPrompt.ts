...
export function buildReviewPrompt(
  ...
  return [
    ...
    "severity is the authoritative signal and the only one the consumer reads. A hedge, caveat, or withdrawal written inside claim or proposed_fix prose is ignored, so a finding you have talked yourself out of must be dropped or emitted below BLOCK/FIX_FIRST, never left at a blocking severity.",
    ...
    "Every finding must be grounded in the current patch or in artifacts under Candidate root. Do not manufacture a finding from USER_INTENT.md, PLAN_ACCEPTED.md, or TESTS.txt text alone. If the supplied intent or test evidence contradicts the patch, that contradiction is itself the finding: claim it and cite both sides.",
    "State the reasoning that makes the claim true, not only the conclusion, so a reader can check it against the cited lines. A proposed_fix whose own justification does not survive checking is worse than no finding, because the author will comply with it.",
    "",

...
async function driveTicks(
  ...
  for (let tick = 0; tick < ticks; tick += 1) {
    ...
    const healthy = registry.read().snapshots.find((s) => s.subject.subject_id === healthyId);
...
describe("QuotaRegistry poll sweep: renewal is never postponed by a sibling's retry ladder (#263)", () => {
  it.each([
    ...
    "%s lane: a healthy profile stays fresh beside a permanently %s sibling, whose absence stays stated",
    async (vendor, source, reason) =>
      withJournal("claudexor-quota-263-", async (journal) => {
        ...
        expect(registry.read().absences.map((a) => a.reason)).toEqual([reason]);
      }),
  );

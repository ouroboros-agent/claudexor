...
function skillText(host: PluginHost, runtime: RuntimePaths): string {
  return [
    ...
    "",
    READINESS_GUIDANCE,
    "An unpinned route is usable only when its doctor-backed status is `ok` and it exposes the requested intent. A named account instead uses its exact `claudexor_accounts` row plus strict pinned preflight; aggregate doctor status or a host/default login neither proves nor vetoes that profile. Reviewer identity: an omitted profile uses the canonical model-first account pool; a structured reviewer entry may carry strict `credentialProfileId` and never silently falls back. Use `--reviewer-panel-json '<array>'` for pinned CLI entries; the compact `--reviewer-panel` spelling remains unpinned.",

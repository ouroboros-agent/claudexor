...
function fakeInteractiveConpty(): TerminalTransportResolution {
  const source = [
    ...
    `  if (!input.includes("\\u001b[79;24;111;1;0;1_") || !input.includes("\\u001b[189;12;45;1;0;1_")) process.exit(4);`,
    `  const code = [...input.matchAll(/\\u001b\\[[0-9]+;[0-9]+;([0-9]+);1;[0-9]+;1_/g)]`,
    `    .map(match => Number(match[1])).filter(codeUnit => codeUnit !== 13)`,
    `    .map(codeUnit => String.fromCharCode(codeUnit)).join("");`,
